...
- If all receivers of an item are above the user-defined healthy %SS threshold, the app moves the balancing week forward one week at a time until at least one receiver is at or below the threshold, or the latest available week is reached.
...
with right:
    ...
    health_threshold_pct = st.number_input(
        ...
        step=5.0,
        help=(
            "Used only when Auto separate Firm PO week and balancing week is enabled. "
            "Default is 150%."
        ),
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            st.session_state["last_run_info"] = {
                ...
                "Healthy %SS Threshold": health_threshold_pct,
