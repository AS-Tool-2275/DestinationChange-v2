...
3. `DueDateCalc.xlsx`. App always uses `due_date` offset mode.
...
- `Priority Rank` can be used to prioritize warehouses from top to bottom.
- `Auto separate firm week and balancing week` will move the balancing week forward automatically when needed.
...
with right:
    ...
    st.caption("DueDate offset mode is fixed to due_date (ceil(Delivery Days / 7)).")
    output_name = st.text_input(
        "Output file name",
        value=f"destination_change_unified_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
    )
    auto_balance_week = st.checkbox(
        "Auto separate firm week and balancing week",
        value=False,
        help="If enabled, the app will auto-push the balancing week forward until the item is not entirely above 150% SS%.",
    )
...
priority_df = st.data_editor(
    ...
    column_config={
        ...
        "Rank": st.column_config.NumberColumn("Rank", help="1 = ưu tiên cao nhất, cùng rank là ngang nhau"),
    },
)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            st.session_state["last_run_info"] = {
                ...
                "Auto separate firm week and balancing week": auto_balance_week,
