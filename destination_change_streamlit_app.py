...
st.write("Leave blank if there are no priority warehouses. `Value`: enter 50 or 0.5 to mean 50%; enter 1 to mean 100%. `Rank` is optional; smaller ranks are prioritized first, and the same rank is treated as equal priority.")
priority_df = st.data_editor(
    ...
    column_config={
        ...
        "Rank": st.column_config.NumberColumn("Rank", help="Optional. Số nhỏ hơn sẽ được ưu tiên trước; cùng rank = ngang nhau"),
    },
)
...
def build_priority_rules(priority_df: pd.DataFrame) -> Dict[str, PriorityRule]:
    ...
    for _, row in priority_df.iterrows():
        ...
        try:
            rank_int = None if pd.isna(rank) or rank == "" else int(float(rank))
        except Exception:
            rank_int = None
        rules[whse] = PriorityRule(whse=whse, mode=mode, value=normalize_pct(value_float), priority_rank=rank_int)
    return rules
