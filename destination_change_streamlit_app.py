...
- Normally the balancing week is the same week.
- If all warehouses of an item are above 150% SS%, the app automatically moves the balancing week forward one week at a time until at least one warehouse is at or below 150%, or the latest available week is reached.
- Warehouse 335 uses accumulated NET FCST from Current Week through the selected balancing week.
"""
        )
...
def render_vendor_mapping(psw_files) -> None:
    ...
    try:
        ...
        due_count = len(st.session_state.get("due_files", []))
...
with left:
    ...
    st.session_state["due_files"] = due_files or []
