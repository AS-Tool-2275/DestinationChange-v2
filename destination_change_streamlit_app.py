...
- If all receivers of an item are above the user-defined Healthy SS% Threshold (default 150%), the app moves the balancing week forward one week at a time until at least one receiver is at or below the threshold, or the latest available week is reached.
...
with right:
    ...
    healthy_ss_threshold_pct = st.number_input(
    ...
    auto_balance_week = st.checkbox(
        ...
        help=(
            ...
            "On: if all receivers of an item remain above the healthy SS% threshold, the app automatically moves the balancing week forward one week at a time while Firm PO remains tied to the selected Firm PO Week."
        ),
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            final_path = process_files(
                ...
                healthy_ss_threshold_pct=healthy_ss_threshold_pct,
            )
            ...
            st.session_state["last_run_info"] = {
                ...
                "Healthy SS% Threshold": f"{healthy_ss_threshold_pct:g}%",
