...
Default behavior:
  ...
  - The default offset mode is legacy compatible to match the current SI-SS_WANEK 3.py logic.
    To use DueDateCalc directly, run with offset_mode="due_date".
"""
...
def load_due_date_offsets(due_date_calc_path: str, offset_mode: str = "legacy_compatible") -> Tuple[Dict[str, int], pd.DataFrame]:
    ...
    if header_idx is None:
        raise ValueError("Could not find the Warehouse / Delivery Days header in DueDateCalc.")
...
def build_vendor_offset_maps(
    ...
    for _, r in timeline_vendor_df.iterrows():
        ...
        if due_path not in cache:
            cache[due_path], _ = load_due_date_offsets(due_path, offset_mode=offset_mode)
...
def build_rows(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> Tuple[List[dict], int]:
    ...
    for _, r in group.iterrows():
        ...
        if rule is not None:
            ...
            # SI = 0 is a hard lock: keep the warehouse at its original F quantity.
            row["hard_lock"] = rule.mode == "SI" and float(rule.value) == 0.0
            row["priority_target_f_after"] = row["orig_f"] if row["hard_lock"] else compute_priority_target_final(row, rule)
        rows.append(row)
...
def choose_priority_recipient(rows: List[dict], priority_indices: List[int]) -> Optional[int]:
    ...
    for idx in priority_indices:
        ...
        rank_sort = int(rank) if rank is not None and pd.notna(rank) else 10**9
        ...
        candidates.append((rank_sort, -gap, primary_metric, secondary_metric, row["whse"], idx))
    if not candidates:
        return None
...
def _fallback_equalize_zero_ss_rows(rows: List[dict]) -> None:
    """Second-pass equalization for rows with SS target equal to zero.

    Goal: minimize spread of SI After among warehouses where Average of SS Wk3 is 0,
    without touching rows that have positive SS targets.
    """
    zero_indices = [i for i, r in enumerate(rows) if float(r.get("ss_target", 0)) <= 0]
    if len(zero_indices) < 2:
        return

    # Rebalance one firm unit at a time from the highest SI After zero-SS row
    # to the lowest SI After zero-SS row until the spread is within 1 unit or no
    # donor has inventory left to give.
    guard = 0
    while guard < 100000:
        guard += 1
        scored = [(current_si_after(rows[i]), i) for i in zero_indices]
        low_si, low_idx = min(scored, key=lambda t: (t[0], rows[t[1]]["whse"]))
        high_si, high_idx = max(scored, key=lambda t: (t[0], rows[t[1]]["whse"]))
        if high_si - low_si <= 1:
            break
        donor_candidates = sorted(
            [i for i in zero_indices if rows[i]["final_f"] > 0],
            key=lambda i: (-current_si_after(rows[i]), rows[i]["whse"]),
        )
        if not donor_candidates:
            break
        donor_idx = donor_candidates[0]
        if donor_idx == low_idx:
            # Fall back to the next donor if the current highest SI row is also the lowest.
            if len(donor_candidates) < 2:
                break
            donor_idx = donor_candidates[1]
        if donor_idx == low_idx or rows[donor_idx]["final_f"] <= 0:
            break
        rows[donor_idx]["final_f"] -= 1
        rows[low_idx]["final_f"] += 1
...
def allocate_item(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> pd.DataFrame:
    ...
    # Hard-lock rows (SI = 0) keep their original F quantity and are excluded from any reallocation step.
    locked_total = 0
    for row in rows:
        if row.get("hard_lock", False):
            row["final_f"] = int(row["orig_f"])
            locked_total += int(row["orig_f"])

    remaining = total_f - locked_total
    if remaining < 0:
        raise ValueError(f"Item {group['Item'].iloc[0]}: hard lock quantity exceeds item total firm PO.")

    priority_indices = [i for i, r in enumerate(rows) if r["priority_rule_mode"] and not r.get("hard_lock", False)]
    non_priority_indices = [i for i, r in enumerate(rows) if not r["priority_rule_mode"] and not r.get("hard_lock", False)]

    # Phase 1: satisfy priority rules first.
    while remaining > 0:
    ...
    # Phase 2: allocate the remaining firm quantities by the SS% rule.
    ...
    # Fallback equalization pass for SS = 0 warehouses.
    apply_zero_ss_equalization(rows)
    ...
    out["Priority Rank"] = [r["priority_rank"] for r in rows]
...
def apply_zero_ss_equalization(rows: List[dict]) -> None:
    """
    Fallback equalization pass for warehouses with Average of SS Wk3 = 0.

    It preserves the total firm quantity of the item and only redistributes one unit at a time
    within the zero-SS group so that the resulting SI After values become closer to each other.
    Locked warehouses are excluded.
    """
    if not rows:
        return
    ...
    zero_idx = [
        ...
        if float(r.get("ss_target", 0.0)) <= 0 and not r.get("hard_lock", False)
    ]
    if len(zero_idx) < 2:
        return

    # Greedy rebalancing: move one unit from the highest SI After row to the lowest SI After row.
    for _ in range(10000):
        candidates = [i for i in zero_idx if int(rows[i].get("final_f", 0)) > 0]
        if len(candidates) < 2:
            break

        donor = max(candidates, key=lambda i: current_si_after(rows[i]))
        recipient = min(candidates, key=lambda i: current_si_after(rows[i]))

        if donor == recipient:
            break

        donor_si = current_si_after(rows[donor])
        recipient_si = current_si_after(rows[recipient])

        if donor_si - recipient_si <= 1:
            break

        rows[donor]["final_f"] -= 1
        rows[recipient]["final_f"] += 1
...
def run_optimizer(
    ...
    if not detail_full.empty and int(detail_full["F Wk3 Original"].sum()) != int(detail_full["F Wk3 After Destination Change"].sum()):
        raise ValueError("Total Firm PO does not match for the full file.")
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        for name, df in (extra_sheets or {}).items():
            ...
            if df is not None and not df.empty:
                df.to_excel(writer, sheet_name=safe_name, index=False)
        logic = pd.DataFrame([
            ["Input PlanDetailTimeline", "Raw CSV; the code automatically skips metadata lines before the Item # header row."],
            ...
            ["F Wk3 After", "Final firm quantity for each warehouse after destination change"],
            ...
            ["Priority mode SS", "Target SS% uses the SI / SS formula"],
            ["Allocation rule", "After priority allocation, always select the warehouse with the lowest SS% After"],
            ["Zero SS fallback", "If an item has multiple warehouses with Average of SS Wk3 = 0, the app applies a second equalization pass to balance SI After within that subset."],
...
def process_files(
    ...
    # DueDateCalc upload order can now follow detected PlanDetailTimeline vendor order.
    # The first file remains the default/main fallback for legacy compatibility.
    due_list = [p for p in (due_date_calc_xlsx_list or []) if p]
    if not due_list:
        due_list = [due_date_calc_xlsx]
        if other_due_date_calc_xlsx:
            due_list.append(other_due_date_calc_xlsx)

    offset_map, offset_debug = load_due_date_offsets(due_list[0], offset_mode=offset_mode)
    ...
    if len(due_list) > 1:
        other_offset_map, other_offset_debug = load_due_date_offsets(due_list[1], offset_mode=offset_mode)
    ...
    # DueDateCalc upload order now follows vendor order detected from PSW / Production Schedule.
    # This is more reliable for multi-vendor files because PSW contains all vendors, while
    # PlanDetailTimeline often shows only the main vendor.
    psw_vendor_df = detect_psw_vendors(all_psw_paths)
    vendor_offset_maps, vendor_due_debug = build_vendor_offset_maps(psw_vendor_df, due_list, offset_mode=offset_mode)
    ...
    run_debug = pd.DataFrame([
        ...
        ["DueDateCalc files uploaded", ", ".join(due_list)],
        ["DueDateCalc mapping rule", "Upload order follows detected PlanDetailTimeline vendor order; if fewer files than vendors, the last file is fallback"],
        ["TargetWeek", fmt_date(target_week)],
        ["CurrentWeek", fmt_date(current_week)],
    ...
    debug_sheets = {
        ...
        "Timeline Vendor Due Mapping": vendor_due_debug,
...
def ask_priority_rules(warehouses: List[str]) -> Dict[str, PriorityRule]:
    ...
    raw = simpledialog.askstring(
        "Priority warehouses",
        "Enter one or more priority warehouses, separated by commas.\n"
        "Example: 11,17,28\n"
        "Leave blank if there is no priority warehouse.",
    )
    if not raw:
        root.destroy()
        return rules
    ...
    for wh in selected:
        mode = simpledialog.askstring(
            f"Rule for warehouse {wh}",
            "Enter the mode for this warehouse:\n- SI: cover part of the gap toward SI = 0\n- SS: cover toward a specific SS% target",
        )
        if not mode:
            continue
        mode = mode.strip().upper()
        rank_val = simpledialog.askinteger(
            f"Priority rank for warehouse {wh}",
            "Optional. Smaller rank is higher priority. Same rank = same priority. Leave blank to keep current behavior.",
        )
        if mode == "SI":
            val = simpledialog.askfloat(f"% SI cover for warehouse {wh}", "Example: 50 = cover 50%, 100 = fully cover to SI=0")
            if val is not None:
                rules[wh] = PriorityRule(whse=wh, mode="SI", value=normalize_pct(val), priority_rank=rank_val)
        elif mode == "SS":
            val = simpledialog.askfloat(f"Target SS% for warehouse {wh}", "Example: 0 = SI equals SS; -100 = SI equals 0; 100 = SI equals 2x SS")
            if val is not None:
                rules[wh] = PriorityRule(whse=wh, mode="SS", value=normalize_pct(val), priority_rank=rank_val)
    root.destroy()
    return rules
    ...
    def run(self):
        try:
            ...
            if not plan or not prod or not due or not out:
                raise ValueError("Please select all 4 file/path inputs: PlanDetailTimeline, Production Schedule, DueDateCalc, and Output.")
            target_week = parse_user_date(self.target_var.get())
            current_week = parse_user_date(self.current_var.get())
            self.status_var.set("Reading warehouse list for priority setup...")
            ...
            self.status_var.set("Processing the full flow...")
            self.root.update_idletasks()
            ...
            messagebox.showinfo("Done", f"Output file created:\n{final_path}")
...
def main():
    ...
    if tk is None:
        raise SystemExit("Tkinter is not available. Please run CLI mode with --no-gui and the required input parameters.")
    root = tk.Tk()
    UnifiedApp(root)
    root.mainloop()
