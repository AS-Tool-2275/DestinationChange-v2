...
OUTPUT_COLUMNS = [
    "Item", "Item Class", "Coll. Class", "Series", "Division", "Item Status", "Future Status", "Hold/ Buy", "ABC", "Source Key",
    "ProdResourceID",
    "Whse",
    "F Wk3",
    "Sum of SI Wk3",
...
def extract_timeline_metadata(raw: pd.DataFrame) -> pd.DataFrame:
    """Return the nine requested PlanDetailTimeline metadata columns at Item + Whse + Coll. Class grain.

    These fields are reference-only and never participate in optimization calculations.
    """
    requested = [
        "Item Class", "Coll. Class", "Series", "Division",
        "Item Status", "Future Status", "Hold/ Buy", "ABC", "Source Key",
    ]
    present = [c for c in requested if c in raw.columns]
    if not present:
        return pd.DataFrame(columns=["Item", "Whse", "Coll. Class"] + [c for c in requested if c != "Coll. Class"])
    tmp = raw.copy()
    tmp["Item"] = tmp["Item #"].map(normalize_item) if "Item #" in tmp.columns else ""
    tmp["Whse"] = tmp["Whse"].map(normalize_whse) if "Whse" in tmp.columns else ""
    if "Coll. Class" not in tmp.columns:
        tmp["Coll. Class"] = ""
    cols = ["Item", "Whse", "Coll. Class"] + [c for c in present if c != "Coll. Class"]
    return tmp[cols].drop_duplicates(subset=["Item", "Whse", "Coll. Class"], keep="first")
...
def build_optimizer_input_direct_from_plan(
    ...
    timeline_meta = extract_timeline_metadata(raw)
    if not timeline_meta.empty:
        if "Coll. Class" not in out.columns and "ProdResourceID" in out.columns:
            out["Coll. Class"] = out["ProdResourceID"]
        out = out.merge(timeline_meta, on=["Item", "Whse", "Coll. Class"], how="left", suffixes=("", "_meta"))
...
def auto_select_balance_week_map(
    raw: pd.DataFrame, date_map: Dict[date, str], first_etd_week: date, firm_week: date,
    current_week: date, offset_map: Dict[str, int], optimizer_base: pd.DataFrame,
    priority_rules: Optional[Dict[str, PriorityRule]] = None, respect_priority_rank: bool = False,
    threshold: float = 1.5,
) -> Tuple[Dict[str, date], pd.DataFrame]:
    """Select balance week using post-destination-change receiver SS% at each candidate week.

    Firm PO remains fixed at firm_week. Inventory metrics are recalculated fresh for each candidate
    balancing week. Only warehouses receiving Firm PO (Net Destination Change > 0) are tested against
    the 150% threshold; donors above 150% do not force the balance week forward.
    """
    priority_rules = priority_rules or {}
    all_weeks = sorted(date_map.keys())
    candidate_weeks = [firm_week] + [d for d in all_weeks if d > firm_week]
    items = raw["Item #"].map(normalize_item).dropna().astype(str).unique().tolist()
    selected = {}
    debug_rows=[]
    supply_cols=[c for c in ["Item","Whse","Main Vendor","Main Vendor F Wk3","Other Vendor Supply","Other Vendor List","Timeline Firm PO","PSW F Used for Reconciliation","Firm PO Reconciliation Gap","Total Supply Added to SI"] if c in optimizer_base.columns]
    firm_supply=optimizer_base[supply_cols].drop_duplicates(["Item","Whse"]).copy()
    for wk in candidate_weeks:
        if len(selected)==len(items): break
        metrics,_,_=compute_plan_metrics_for_week_from_raw(raw,date_map,first_etd_week,wk,current_week,offset_map)
        metrics["Item"]=metrics["Item"].astype(str)
        metrics["Whse"]=metrics["Whse"].map(normalize_whse)
        metrics=metrics.merge(firm_supply,on=["Item","Whse"],how="left",suffixes=("","_firm"))
        for c in ["Main Vendor F Wk3","Other Vendor Supply","Firm PO Reconciliation Gap","PSW F Used for Reconciliation","Timeline Firm PO","Total Supply Added to SI"]:
            if c not in metrics.columns: metrics[c]=0.0
            metrics[c]=pd.to_numeric(metrics[c],errors="coerce").fillna(0.0)
        if "Other Vendor List" not in metrics.columns: metrics["Other Vendor List"]=""
        metrics["F Wk3"]=metrics["Main Vendor F Wk3"]
        metrics["Total Supply Added to SI"]=metrics["Main Vendor F Wk3"]+metrics["Other Vendor Supply"]+metrics["Firm PO Reconciliation Gap"]
        metrics["Original SI Before Supply"]=metrics["Current SI"]
        metrics["Original SI-SS Before Supply"]=metrics["Current SI-SS"]
        metrics["New SI"]=metrics["Current SI"]+metrics["Total Supply Added to SI"]
        metrics["New SI-SS"]=metrics["Current SI-SS"]+metrics["Total Supply Added to SI"]
        metrics["Sum of SI Wk3"]=metrics["New SI"]
        metrics["Sum of SI-SS Wk3"]=metrics["New SI-SS"]
        metrics["Current SI"]=metrics["New SI"]
        metrics["Average of SS Wk3"]=metrics["SS_Wk3"]
        metrics["Current SS%"]=metrics.apply(lambda r:safe_ss_ratio(float(r["Current SI"]),float(r["Average of SS Wk3"])),axis=1)
        metrics["Firm PO Total"]=metrics.groupby("Item")["F Wk3"].transform("sum")
        for item,grp in metrics.groupby("Item",sort=False):
            if item in selected: continue
            item_rules={wh:rule for wh,rule in priority_rules.items() if wh in set(grp["Whse"].astype(str))}
            allocated=allocate_item(grp.copy(),item_rules,respect_priority_rank=respect_priority_rank)
            receivers=allocated[allocated["Net Destination Change"].astype(float)>0]
            rec_ss=receivers["SS % After"].astype(float).tolist()
            over=[v for v in rec_ss if v>threshold]
            finite=[v for v in rec_ss if math.isfinite(v)]
            if not over:
                selected[item]=wk
                debug_rows.append([item,fmt_date(wk),min(finite) if finite else None,max(finite) if finite else (math.inf if rec_ss else None),len(grp),"Firm week / selected" if wk==firm_week else "Moved forward / selected"])
    fallback=candidate_weeks[-1]
    for item in items:
        if item not in selected:
            selected[item]=fallback
            debug_rows.append([item,fmt_date(fallback),None,None,None,"Last available week fallback"])
    return selected,pd.DataFrame(debug_rows,columns=["Item","Selected Balance Week","Min Receiver SS% After","Max Receiver SS% After","Warehouse Count","Reason"])
...
def build_optimizer_input_auto_balance(
    ...
    timeline_meta = extract_timeline_metadata(raw)
    if not timeline_meta.empty:
        out = out.merge(timeline_meta, on=["Item","Whse","Coll. Class"], how="left") if "Coll. Class" in out.columns else out.merge(timeline_meta.drop(columns=["Coll. Class"]), on=["Item","Whse"], how="left")
...
def build_detail_output(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    preferred = [
        "Item", "Item Class", "Coll. Class", "Series", "Division", "Item Status", "Future Status", "Hold/ Buy", "ABC", "Source Key",
        "ProdResourceID", "Whse", "F Wk3", "Sum of SI Wk3", "Sum of SI-SS Wk3", "Average of SS Wk3",
...
def process_files(
    ...
    # Optional automatic balancing week. Default behavior keeps the Firm PO week as the balancing week.
    # When enabled, an item moves to later available weeks only if ALL warehouses are above 150% SS.
    # The Firm PO allocation itself remains tied to the selected Firm PO week.
    raw, date_map, first_original_week, last_original_week, first_etd_week = load_plan_source(plan_detail_csv)
    selected_map = {item: target_week for item in raw["Item #"].map(normalize_item).dropna().astype(str).unique().tolist()}
    ...
    ], columns=["Item", "Selected Balance Week", "Min Current SS%", "Max Current SS%", "Warehouse Count", "Reason"])

    if auto_balance_week:
        selected_map, balance_debug = auto_select_balance_week_map(
            raw, date_map, first_etd_week, target_week, current_week, offset_map,
            optimizer_base=optimizer_base, priority_rules=priority_rules,
            respect_priority_rank=respect_priority_rank, threshold=1.5
        )

        if any(wk != target_week for wk in selected_map.values()):
            # Rebuild inventory metrics at the automatically selected balancing week
            # while retaining Firm PO / reconciliation quantities from the selected Firm PO week.
            frames = []
            for balance_week in sorted(set(selected_map.values())):
                ...
                metrics["Item"] = metrics["Item"].astype(str)
                items = [i for i, wk in selected_map.items() if wk == balance_week]
                sub = metrics[metrics["Item"].isin(items)].copy()
                ...
                supply_cols = [
                    ...
                    ] if c in optimizer_base.columns
                ]
                sub = sub.merge(optimizer_base[supply_cols].drop_duplicates(), on=["Item", "Whse"], how="left")

                for c in ["Main Vendor F Wk3", "Other Vendor Supply", "Timeline Firm PO",
                ...
                sub["F Wk3"] = sub["Main Vendor F Wk3"]
                sub["PSW F Used for Reconciliation"] = sub["Main Vendor F Wk3"] + sub["Other Vendor Supply"]
                sub["Total Supply Added to SI"] = (
                    sub["Main Vendor F Wk3"] + sub["Other Vendor Supply"] + sub["Firm PO Reconciliation Gap"]
                )
                sub["Original SI Before Supply"] = sub["Current SI"]
                sub["Original SI-SS Before Supply"] = sub["Current SI-SS"]
                sub["New SI"] = sub["Current SI"] + sub["Total Supply Added to SI"]
                ...
                sub["Sum of SI-SS Wk3"] = sub["New SI-SS"]
                sub["Current SI"] = sub["New SI"]
                sub["Average of SS Wk3"] = pd.to_numeric(sub["SS_Wk3"], errors="coerce").fillna(0.0)
                sub["Current SS%"] = sub.apply(
                    lambda r: safe_ss_ratio(float(r["Current SI"]), float(r["Average of SS Wk3"])), axis=1
                )
                sub["Firm PO Total"] = sub.groupby("Item")["F Wk3"].transform("sum")
                sub["Balance Week"] = balance_week
                frames.append(sub)
    ...
    run_debug = pd.DataFrame([
        ...
        ["Auto balancing rule", "If enabled: keep Firm PO Week unless all warehouses for the item are above 150% SS; then move the balancing week forward until at least one warehouse is at or below 150%"],
