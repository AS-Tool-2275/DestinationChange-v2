...
def compute_plan_metrics_for_week_from_raw(
    ...
    out["Average of SS Wk3"] = out["SS_Wk3"]
    out["Current SS%"] = out.apply(lambda r: safe_ss_ratio(float(r["Current SI"]), float(r["Average of SS Wk3"])), axis=1)
...
def auto_select_balance_week_map(
    ...
    # Start at the week after Firm PO week. If the firm week itself is healthy, keep it.
    all_weeks = sorted(date_map.keys())
    later = [d for d in all_weeks if d > firm_week]
    candidate_weeks = [firm_week] + later if later else [firm_week]
    ...
    for wk in candidate_weeks:
        ...
        for item, grp in metrics.groupby("Item", sort=False):
            if item in selected:
                continue
            eligible = grp[pd.to_numeric(grp["SS_Wk3"], errors="coerce").fillna(0) > 0]
            # Zero-SS warehouses are not evaluated against the percentage threshold;
            # they are handled later by the SI equalization fallback pass.
            all_above = bool(len(eligible) > 0 and (eligible["Current SS%"] > threshold).all())
            if not all_above:
                selected[item] = wk
                finite_vals = [float(x) for x in grp["Current SS%"].tolist() if math.isfinite(float(x))]
                debug_rows.append([item, fmt_date(wk), min(finite_vals) if finite_vals else None, max(finite_vals) if finite_vals else None, len(grp), "Firm week" if wk == firm_week else "Moved forward"])
    ...
    return selected, pd.DataFrame(debug_rows, columns=["Item", "Selected Balance Week", "Min Current SS%", "Max Current SS%", "Warehouse Count", "Reason"])
...
def process_files(
    ...
    # Always determine the balancing week automatically: use Firm Week unless all warehouses of the item
    # are above 150% SS, then move forward week-by-week until at least one warehouse is no longer above 150%.
    raw, date_map, first_original_week, last_original_week, first_etd_week = load_plan_source(plan_detail_csv)
    selected_map, balance_debug = auto_select_balance_week_map(raw, date_map, first_etd_week, target_week, current_week, offset_map, threshold=1.5)

    if any(wk != target_week for wk in selected_map.values()):
        # Rebuild metrics per selected balance week while retaining the firm-week supply/reconciliation values.
        frames = []
        for balance_week in sorted(set(selected_map.values())):
            metrics, _, _ = compute_plan_metrics_for_week_from_raw(raw, date_map, first_etd_week, balance_week, current_week, offset_map)
            metrics["Item"] = metrics["Item"].astype(str)
            items = [i for i, wk in selected_map.items() if wk == balance_week]
            sub = metrics[metrics["Item"].isin(items)].copy()
            supply_cols = [c for c in ["Item", "Whse", "Main Vendor", "Main Vendor F Wk3", "Other Vendor Supply", "Other Vendor List", "Timeline Firm PO", "PSW F Used for Reconciliation", "Firm PO Reconciliation Gap"] if c in optimizer_base.columns]
            sub = sub.merge(optimizer_base[supply_cols].drop_duplicates(), on=["Item", "Whse"], how="left")
            for c in ["Main Vendor F Wk3", "Other Vendor Supply", "Timeline Firm PO", "PSW F Used for Reconciliation", "Firm PO Reconciliation Gap"]:
                if c not in sub.columns:
                    sub[c] = 0.0
                sub[c] = pd.to_numeric(sub[c], errors="coerce").fillna(0.0)
            if "Other Vendor List" not in sub.columns:
            ...
            sub["Firm PO Total"] = sub.groupby("Item")["F Wk3"].transform("sum")
            frames.append(sub)
        optimizer_input = pd.concat(frames, ignore_index=True) if frames else optimizer_base
    else:
        optimizer_input = optimizer_base
    ...
    run_debug = pd.DataFrame([
        ...
        ["Auto balancing rule", "Use Firm Week unless all warehouses for the item are above 150% SS; otherwise move to the next available week until at least one is not above 150%"],
