...
def get_plan_week_range(plan_csv_path: str) -> Tuple[Optional[date], Optional[date]]:
    """Return the min/max date-like week headers from a raw PlanDetailTimeline CSV."""
...
def evaluate_balance_health(detail: pd.DataFrame, threshold: float = 1.5) -> Tuple[bool, pd.DataFrame]:
    """Return whether every item has at least one warehouse at or below the threshold."""
    if detail is None or detail.empty or "Item" not in detail.columns or "SS % After" not in detail.columns:
        return True, pd.DataFrame(columns=["Item", "Min SS % After", "Status"])

    rows = []
    all_ok = True
    for item, g in detail.groupby("Item", sort=True):
        vals: List[float] = []
        for v in pd.to_numeric(g["SS % After"], errors="coerce").tolist():
            if pd.isna(v):
                continue
            try:
                fv = float(v)
            ...
            if math.isinf(fv):
                continue
            vals.append(fv)
        min_val = min(vals) if vals else math.inf
        ok = min_val <= threshold
        all_ok = all_ok and ok
        rows.append({
            "Item": item,
            "Min SS % After": min_val if not math.isinf(min_val) else None,
            "Status": "OK" if ok else f"All warehouses > {threshold*100:.0f}%",
        })
...
def build_optimizer_input_direct_from_plan(
    ...
    # Detect week columns by header date instead of fixed positions.
    date_cols: List[str] = []
...
def process_files(
    ...
    target_week is the firm week used to read Production Schedule F Wk3.
    If auto_balance_week is True, the app searches later balancing weeks
    (starting from target_week + 7 days) until every item has at least one
    warehouse at or below the healthy SS% threshold.
    """
    ...
    # Firm PO week is the user-selected target week.
    firm_week = target_week
    f_wk3, prod_debug = load_fwk3_from_production(production_schedule_csv, firm_week)

    # Determine available weeks in PlanDetailTimeline for balancing-week search.
    plan_first_week, plan_last_week = get_plan_week_range(plan_detail_csv)

    def _run_for_balance_week(balance_week: date):
        optimizer_input, build_debug, missing_f, plan_offset_debug = build_optimizer_input_direct_from_plan(
            plan_detail_csv, offset_map, f_wk3, balance_week, current_week
        )
        detail, summary = run_optimizer(optimizer_input, priority_rules=priority_rules)
        healthy, health_debug = evaluate_balance_health(detail)
        return {
            "balance_week": balance_week,
            "optimizer_input": optimizer_input,
            "build_debug": build_debug,
            "missing_f": missing_f,
            "plan_offset_debug": plan_offset_debug,
            "detail": detail,
            "summary": summary,
            "healthy": healthy,
    ...
    # Default behavior: balance on the same week as the firm week.
    chosen_balance_week = firm_week
    chosen_result = _run_for_balance_week(chosen_balance_week)

    auto_history_rows = []
    if auto_balance_week:
        search_start = firm_week + timedelta(days=7)
        if plan_last_week is not None and search_start <= plan_last_week:
            candidate_weeks = date_range_saturdays(search_start, plan_last_week)
        else:
            candidate_weeks = []

        # If no later weeks are available, keep the default result.
        if candidate_weeks:
            for candidate in candidate_weeks:
                result = _run_for_balance_week(candidate)
                auto_history_rows.append({
                    "Candidate Balance Week": fmt_date(candidate),
                    "Healthy": "Yes" if result["healthy"] else "No",
                    "Items Requiring Next Week": int((result["health_debug"]["Status"] != "OK").sum()) if not result["health_debug"].empty else 0,
                })
                chosen_balance_week = candidate
                chosen_result = result
                if result["healthy"]:
                    break

    detail = chosen_result["detail"]
    summary = chosen_result["summary"]
    optimizer_input = chosen_result["optimizer_input"]
    build_debug = chosen_result["build_debug"]
    missing_f = chosen_result["missing_f"]
    plan_offset_debug = chosen_result["plan_offset_debug"]
    ...
    run_debug = pd.DataFrame([
        ...
        ["Firm Week / Production Week", fmt_date(firm_week)],
        ["Selected Balancing Week", fmt_date(chosen_balance_week)],
        ["Auto balance week", "Yes" if auto_balance_week else "No"],
        ["CurrentWeek", fmt_date(current_week)],
    ...
    final_path = write_excel_output(output_path, detail, summary, optimizer_input, debug_sheets)

    if return_info:
        info = {
            "Firm Week": fmt_date(firm_week),
            "Selected Balancing Week": fmt_date(chosen_balance_week),
            "Auto balance week": auto_balance_week,
            "Current Week": fmt_date(current_week),
            "Offset mode": offset_mode,
            "Total F original": int(detail["F Wk3 Original"].sum()) if not detail.empty else 0,
            "Total F after": int(detail["F Wk3 After Destination Change"].sum()) if not detail.empty else 0,
            "Auto history": auto_history_rows,
            "Plan date range": (
                fmt_date(plan_first_week) if plan_first_week else None,
                fmt_date(plan_last_week) if plan_last_week else None,
            ),
        }
