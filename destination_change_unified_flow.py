...
Default behavior:
  ...
  - Warehouse offsets are calculated directly from DueDateCalc using ceil(Delivery Days / 7).
"""
...
def build_optimizer_input_direct_from_plan(
    ...
    out = add_timeline_metadata(raw, out)
    if vendor_col:
...
def add_timeline_metadata(raw: pd.DataFrame, out: pd.DataFrame) -> pd.DataFrame:
    """Attach selected PlanDetailTimeline master attributes by Item + Whse + Coll. Class."""
    metadata_cols = [
        "Item Class", "Coll. Class", "Series", "Division",
        "Item Status", "Future Status", "Currency Code", "Price",
        "Hold/ Buy", "ABC", "Key Item", "Fcst. Type",
        "Derived Fcst. Factor", "Source Key",
    ]
    key_cols = ["Item #", "Whse", "Coll. Class"]
    available = [c for c in metadata_cols if c in raw.columns and c not in key_cols]
    if not available or out is None or out.empty:
        return out

    meta = raw[key_cols + available].copy()
    for c in available:
        meta[c] = meta[c].replace({pd.NA: "", None: ""}).fillna("").astype(str).str.strip()
    # Keep the first non-blank value for each Item / Whse / Coll. Class.
    def first_nonblank(series):
        for value in series:
            ...
            if text and text.lower() != "nan":
                return text
        return ""
    agg = {c: first_nonblank for c in available}
    meta = meta.groupby(key_cols, dropna=False, as_index=False).agg(agg)
    merged = out.merge(meta, on=key_cols, how="left", suffixes=("", "_meta"))
    for c in available:
        mc = f"{c}_meta"
        if mc in merged.columns:
            if c in merged.columns:
                blank = merged[c].isna() | merged[c].astype(str).str.strip().eq("")
                merged.loc[blank, c] = merged.loc[blank, mc]
                merged = merged.drop(columns=[mc])
            else:
                merged = merged.rename(columns={mc: c})
    return merged
...
def compute_plan_metrics_for_week_from_raw(
    ...
    out = add_timeline_metadata(df, out)
...
def build_detail_output(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    # Coll. Class is already the internal ProdResourceID; expose the business label as a separate output column.
    if "Coll. Class" not in out.columns and "ProdResourceID" in out.columns:
        out["Coll. Class"] = out["ProdResourceID"]
        # Keep it close to ProdResourceID in the exported layout.
        cols = list(out.columns)
        if "ProdResourceID" in cols:
            cols.remove("Coll. Class")
            insert_at = cols.index("ProdResourceID") + 1
            cols.insert(insert_at, "Coll. Class")
            out = out[cols]
...
def process_files(
    ...
    healthy_ss_threshold_pct: float = 150.0,
) -> str:
    ...
    if auto_balance_week:
        selected_map, balance_debug = auto_select_balance_week_map(
            ...
            threshold=max(0.0, float(healthy_ss_threshold_pct)) / 100.0,
        )
    ...
    run_debug = pd.DataFrame([
        ...
        ["Auto balancing rule", f"If enabled: keep Firm PO Week unless every receiver after destination change is above {float(healthy_ss_threshold_pct):g}% SS; then move balancing week forward one week at a time. Inventory metrics are recalculated fresh at each balancing week."],
