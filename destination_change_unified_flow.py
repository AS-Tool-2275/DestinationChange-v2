...
def _pick_firm_po_column(df: pd.DataFrame, preferred: str = "F Wk3_from_prod") -> pd.Series:
    """Return the merged firm quantity column robustly, even if suffixes differ."""
    if df is None or df.empty:
        return pd.Series(dtype=float)
    for col in (preferred, "F Wk3", "F Wk3_from_prod", "Firm PO", "Firm PO_from_prod"):
...
def load_due_date_offsets(due_date_calc_path: str, offset_mode: str = "due_date") -> Tuple[Dict[str, int], pd.DataFrame]:
    """
    offset_mode:
...
def build_optimizer_input_direct_from_plan(
    ...
    out["F Wk3"] = _pick_firm_po_column(out)
    if "F Wk3_from_prod" in out.columns:
        out = out.drop(columns=["F Wk3_from_prod"])

    # Business rule for optimizer input:
    # Production Schedule F Wk3 is already an inbound firm quantity for the target week.
    # Therefore optimizer must use inventory AFTER adding this F Wk3.
    # New SI = current SI + F Wk3
    # New SI-SS = current SI-SS + F Wk3
    out["Sum of SI Wk3"] = pd.to_numeric(out["Sum of SI Wk3"], errors="coerce").fillna(0.0) + out["F Wk3"]
    out["Sum of SI-SS Wk3"] = pd.to_numeric(out["Sum of SI-SS Wk3"], errors="coerce").fillna(0.0) + out["F Wk3"]
...
# ============================================================
# Auto balancing helpers (separate firm week and balance week)
# ============================================================
...
def load_plan_source(plan_csv_path: str) -> Tuple[pd.DataFrame, Dict[date, str], date, date, date]:
    ...
    last_original_week = max(original_dates)
    first_etd_week = first_original_week - timedelta(days=154)
    return raw, date_map, first_original_week, last_original_week, first_etd_week
...
def compute_plan_metrics_for_week_from_raw(
    ...
    unknown_whse = sorted(set(df.loc[~df["Whse"].isin(offset_map.keys()), "Whse"].dropna().astype(str)))

    planned_etd_weeks = date_range_saturdays(first_etd_week, eval_week)
    net_etd_weeks = date_range_saturdays(current_week, eval_week)

    for offset, idx in df.groupby("_offset_weeks", sort=False).groups.items():
        target_src = eval_week + timedelta(days=7 * int(offset))
        target_col = date_map.get(target_src)
        if target_col:
        ...
        net_cols = [date_map[d + timedelta(days=7 * int(offset))] for d in net_etd_weeks if (d + timedelta(days=7 * int(offset))) in date_map]
        if net_cols:
            df.loc[idx, "_net_sum"] = df.loc[idx, net_cols].apply(pd.to_numeric, errors="coerce").fillna(0.0).sum(axis=1).values

    key_cols = ["Item #", "Whse", "Coll. Class"]
    si_g = group_value(df[df["Data Type"] == "SHIPPABLE INV"], key_cols, "_target_value", "Base_SI")
    planned_g = group_value(df[df["Data Type"] == "PLANNED POS"], key_cols, "_planned_sum", "PlannedPO_Sum")
    firm_g = group_value(df[df["Data Type"] == "FIRM POS"], key_cols, "_target_value", "FirmPO_Target")
    net_g = group_value(df[df["Data Type"] == "NET FCST"], key_cols, "_net_sum", "NetFcst_Sum")
    ss_g = group_value(df[df["Data Type"] == "SAFETY STK"], key_cols, "_target_value", "SS_Wk3")

    base = df[df["Data Type"].isin(["SHIPPABLE INV", "PLANNED POS", "FIRM POS", "NET FCST", "SAFETY STK"])][key_cols].drop_duplicates()
    out = base.merge(si_g, on=key_cols, how="left").merge(planned_g, on=key_cols, how="left").merge(firm_g, on=key_cols, how="left").merge(net_g, on=key_cols, how="left").merge(ss_g, on=key_cols, how="left")
    for col in ["Base_SI", "PlannedPO_Sum", "FirmPO_Target", "NetFcst_Sum", "SS_Wk3"]:
        out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0)
    ...
    if vendor_col:
        vendor_map = df.groupby(key_cols, dropna=False)[vendor_col].first().reset_index().rename(columns={vendor_col: "Vendor"})
        out = out.merge(vendor_map, on=key_cols, how="left")
    else:
        out["Vendor"] = ""
    ...
    debug = pd.DataFrame([
        ["Evaluation week", fmt_date(eval_week)],
        ["Current week", fmt_date(current_week)],
        ["Plan rows after MakeBuy B", len(df)],
        ["Unknown Whse count", len(unknown_whse)],
        ["Unknown Whse list", ", ".join(unknown_whse)],
        ["Rows output", len(out)],
    ], columns=["Field", "Value"])
...
def auto_select_balance_week_map(
    ...
    candidate_weeks = [d for d in sorted(date_map.keys()) if d >= firm_week]
    if not candidate_weeks:
        candidate_weeks = [firm_week]
    ...
    items = raw["Item #"].dropna().astype(str).unique().tolist()
    ...
    for wk in candidate_weeks:
        ...
        metrics["Current SS%"] = metrics.apply(lambda r: safe_ss_ratio(float(r["Current SI"]), float(r["SS_Wk3"])), axis=1)
        for item, grp in metrics.groupby("Item", sort=False):
            if item in selected:
                continue
            if (grp["Current SS%"] > threshold).all():
                continue
            selected[item] = wk
            debug_rows.append([item, fmt_date(wk), float(grp["Current SS%"].min()), float(grp["Current SS%"].max()), len(grp)])
        if len(selected) == len(items):
            break

    for item in items:
        if item not in selected:
            selected[item] = last_week
            debug_rows.append([item, fmt_date(last_week), None, None, None])

    debug = pd.DataFrame(debug_rows, columns=["Item", "Selected Balance Week", "Min Current SS%", "Max Current SS%", "Warehouse Count"])
...
def build_optimizer_input_auto_balance(
    ...
    selected_week_map, balance_debug = auto_select_balance_week_map(raw, date_map, first_etd_week, target_week, current_week, offset_map, threshold=threshold)

    unique_weeks = sorted(set(selected_week_map.values()))
    frames = []
    all_missing = []
    build_debug_rows = []
    plan_offset_rows = []
    for eval_week in unique_weeks:
        metrics, week_debug, offset_by_whse = compute_plan_metrics_for_week_from_raw(raw, date_map, first_etd_week, eval_week, current_week, offset_map)
        metrics["Item"] = metrics["Item"].astype(str)
        items_for_week = [item for item, wk in selected_week_map.items() if wk == eval_week]
        sub = metrics[metrics["Item"].isin(items_for_week)].copy()
        sub = sub.merge(f_wk3, on=["Item", "Whse"], how="left", suffixes=("", "_from_prod"))
        sub["F Wk3"] = _pick_firm_po_column(sub)
        if "F Wk3_from_prod" in sub.columns:
            sub = sub.drop(columns=["F Wk3_from_prod"])
        sub["Sum of SI Wk3"] = sub["Current SI"] + sub["F Wk3"]
        sub["Sum of SI-SS Wk3"] = sub["Current SI-SS"] + sub["F Wk3"]
        frames.append(sub)

        merge_debug = sub.merge(f_wk3, on=["Item", "Whse"], how="left", indicator=True, suffixes=("", "_prod"))
        missing_f = merge_debug[merge_debug["_merge"] == "left_only"][["Item", "Whse", "ProdResourceID"]].drop_duplicates()
        if not missing_f.empty:
            all_missing.append(missing_f)
        build_debug_rows.append([fmt_date(eval_week), len(sub), float(sub["F Wk3"].sum()), len(items_for_week)])
        plan_offset_rows.extend(offset_by_whse.assign(EvaluationWeek=fmt_date(eval_week)).to_dict("records"))

    out = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
    for col in ["Sum of SI Wk3", "Sum of SI-SS Wk3", "Average of SS Wk3", "Current SI", "Current SI-SS", "Current SS%", "F Wk3"]:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0)

    out["Firm PO Total"] = out.groupby("Item", dropna=False)["F Wk3"].transform("sum") if not out.empty else 0.0
    out["Current SI After"] = out["Current SI"] if not out.empty else pd.Series(dtype=float)
    out["SS % After"] = out["Current SS%"] if not out.empty else pd.Series(dtype=float)
    build_debug = pd.DataFrame(build_debug_rows, columns=["Balance Week", "Rows", "Firm PO Total", "Items"])
    plan_offset_debug = pd.DataFrame(plan_offset_rows)
    missing_f = pd.concat(all_missing, ignore_index=True).drop_duplicates() if all_missing else pd.DataFrame(columns=["Item", "Whse", "ProdResourceID"])
    return out, build_debug, missing_f, plan_offset_debug, balance_debug
# ============================================================
...
def write_excel_output(
    ...
    """Write the final production output.

    Streamlit production version intentionally exports only the Optimized Data
    sheet. Intermediate/debug sheets are still built internally, but are not
    included in the downloaded workbook.
    """
...
def process_files(
    ...
    if auto_balance_week:
        optimizer_input, build_debug, missing_f, plan_offset_debug, balance_debug = build_optimizer_input_auto_balance(
        ...
        balance_debug = pd.DataFrame(columns=["Item", "Selected Balance Week", "Min Current SS%", "Max Current SS%", "Warehouse Count"])
        auto_label = "Disabled"
    ...
    run_debug = pd.DataFrame([
        ...
        ["Auto separate firm week and balancing week", auto_label],
    ...
    debug_sheets = {
        ...
        "Plan Convert Debug": pd.DataFrame([ ["Conversion mode", "Direct fast path, equivalent to ETA -> ETD shift without writing converted workbook"], ["Materialized converted workbook", "No"] ], columns=["Field", "Value"]),
...
class UnifiedApp:
    def __init__(self, root):
        ...
        self.offset_mode_var = tk.StringVar(value="due_date")
    ...
    def build_ui(self):
        ...
        ttk.Combobox(dates, textvariable=self.offset_mode_var, values=["due_date", "due_date"], state="readonly", width=24).grid(row=2, column=1, sticky="w", padx=10, pady=10)
...
def main():
    ...
    parser.add_argument("--offset-mode", choices=["due_date", "due_date"], default="due_date")
