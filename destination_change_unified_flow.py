...
TIMELINE_METADATA_COLUMNS = [
    "Item Class", "Coll. Class", "Series", "Division", "Item Status", "Future Status",
    "Hold/ Buy", "ABC", "Source Key",
]
...
def build_optimizer_input_direct_from_plan(
    ...
    # Preserve PlanDetailTimeline master-data attributes at the Item + Whse level for
    # planner reference/debugging. Values are taken from the first non-blank record
    # for each Item + Whse.
    available_metadata = [c for c in TIMELINE_METADATA_COLUMNS if c in raw.columns]
    metadata_key = ["Item #", "Whse"]
    timeline_metadata = raw[metadata_key + available_metadata].copy()
    if available_metadata:
        for c in available_metadata:
            timeline_metadata[c] = timeline_metadata[c].replace({"nan": "", "NaN": ""}).fillna("").astype(str).str.strip()
        timeline_metadata = (
            timeline_metadata.groupby(metadata_key, dropna=False)[available_metadata]
            .agg(lambda col: next((v for v in col if str(v).strip()), ""))
            .reset_index()
        )
    else:
        timeline_metadata = raw[metadata_key].drop_duplicates().copy()
    ...
    if available_metadata:
        metadata_merge = timeline_metadata.copy()
        metadata_merge["Item"] = metadata_merge["Item #"].map(normalize_item)
        metadata_merge["Whse"] = metadata_merge["Whse"].map(normalize_whse)
        metadata_merge = metadata_merge.drop(columns=["Item #"])
        out = out.merge(metadata_merge, on=["Item", "Whse"], how="left")
    else:
        for c in TIMELINE_METADATA_COLUMNS:
            out[c] = ""
...
def build_detail_output(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    preferred = [
        ...
        "Item Class", "Coll. Class", "Series", "Division", "Item Status", "Future Status",
        "Currency Code", "Price", "Hold/ Buy", "ABC", "Key Item", "Fcst. Type",
...
def process_files(
    ...
    health_threshold_pct: float = 150.0,
) -> str:
    ...
    try:
        health_threshold_pct = float(health_threshold_pct)
    except Exception as exc:
        raise ValueError(f"Health %SS must be a number: {exc}")
    if health_threshold_pct <= 0:
        raise ValueError("Health %SS must be greater than 0.")
    health_threshold = health_threshold_pct / 100.0
    ...
    run_debug = pd.DataFrame([
        ...
        ["Health %SS threshold", f"{health_threshold_pct:.1f}%"],
        ["Auto balancing rule", "If enabled: keep Firm PO Week unless every receiver after destination change is above the user-defined healthy %SS threshold; then move balancing week forward one week at a time. Inventory metrics are recalculated fresh at each balancing week."],
